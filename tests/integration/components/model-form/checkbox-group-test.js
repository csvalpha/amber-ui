...
import component from 'alpha-amber/tests/pages/components/model-form/checkbox-group';
...
moduleForComponent('model-form/checkbox-group', 'Integration | Component | model form/checkbox group', {
  integration: true,
  beforeEach() {
...
test('it lists all given options', (assert) => {
  page.render(hbs`{{model-form/checkbox-group model=model property='testProperty' options=options}}`);
...
test('it shows the current model property value when multiple are selected', function(assert) {
  this.set('model', MockModel.create({
    testProperty: [2, 3]
  }));
  page.render(hbs`{{model-form/checkbox-group model=model property='testProperty' options=options}}`);
  assert.notOk(page.options(0).checked);
  assert.ok(page.options(1).checked);
  assert.ok(page.options(2).checked);
});
...
test('it uses the given element id', (assert) => {
  assert.expect(9);
  page.render(hbs`{{model-form/checkbox-group model=model property='testProperty' options=options inputIdentifier='a-very-cool-id'}}`);
  [0, 1, 2].forEach(i => {
...
test('it shows the given label', (assert) => {
  page.render(hbs`{{model-form/checkbox-group model=model property='testProperty' label='TESTLABEL'}}`);
...
test('it makes the input elements disabled', (assert) => {
  ...
  page.render(hbs`{{model-form/checkbox-group model=model property='testProperty' options=options disabled=true}}`);
  [0, 1, 2].forEach(i => {
...
test('it makes the input elements required when the property is required', (assert) => {
  assert.expect(3);
  page.render(hbs`{{model-form/checkbox-group model=model property='testProperty' options=options required=true}}`);
  [0, 1, 2].forEach(i => {
...
test('it adds an asterix (*) to the label when the property is required', (assert) => {
  page.render(hbs`{{model-form/checkbox-group model=model property='testProperty' required=true label='TESTLABEL'}}`);
  assert.equal(page.label, 'TESTLABEL *');
});
...
test('it shows the errors when the models has errors', function(assert) {
  ...
  page.render(hbs`{{model-form/checkbox-group model=model property='testProperty'}}`);
...
test('it sets the current model property value', function(assert) {
  ...
  page.render(hbs`{{model-form/checkbox-group model=model property='testProperty' options=options}}`);
  assert.notOk(this.get('model.testProperty').includes(1));
  assert.ok(this.get('model.testProperty').includes(2));
  assert.ok(this.get('model.testProperty').includes(3));

  page.options(0).check();
  assert.ok(this.get('model.testProperty').includes(1));
  assert.ok(this.get('model.testProperty').includes(2));
  assert.ok(this.get('model.testProperty').includes(3));
  ...
  assert.ok(this.get('model.testProperty').includes(1));
  assert.ok(this.get('model.testProperty').includes(2));
  assert.notOk(this.get('model.testProperty').includes(3));
});
