...
export default class PollsShowController extends Controller {
  ...
  @computed(
    'model.poll.question.options.@each.option',
    'model.poll.question.options.@each.sumOfAnswers',
    'model.poll.form.amountOfResponses'
  )
  get optionsWithResults() {
    ...
    return options
      .map((option) => {
        ...
        const percentage =
          (option.get('sumOfAnswers') / amountOfResponses) * 100;
  ...
  submitResponse() {
    ...
    currentUserResponse
      ...
      .catch((error) => {
        ...
        if (
          error.errors &&
          error.errors.isAny('source.pointer', '/data/relationships/user')
        ) {
          this.errorMessage =
            'Er is al een keuze gevonden, probeer eerst te refreshen, zie je dit formulier dan nog? Neem dan contact op met de ict-commissie.';
        }
      });
  }
}
